...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int result = intArray[0][column];
	   for (int row = 0; row < intArray.length; row++) {
		   if (intArray[row][column] < result) {
			   result = intArray[row][column];
		   }
		   animate(row, column, result);
	   }

      return result; // replace this line with your return statement
   }
